...
require 'new_source/components/modalUploadImage.php';
...
function iap_referal_init() {
	...
	if( $post->post_type == "product" ) {
		...
		$product = $_pf->get_product($product_id = $post->ID);
		// var_dump($post);
		$dir = plugin_dir_path( __FILE__ );
		include($dir."new_source/index.php");
		die();
	}
}
...
function iap_register_scripts(){
	wp_enqueue_style('estilo', plugins_url('PedidoPersonalizadoInstaarts/new_source/css/style.css'));
}
